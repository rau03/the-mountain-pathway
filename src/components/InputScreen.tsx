...
export const InputScreen: React.FC<InputScreenProps> = ({ step }) => {
  ...
  // shifts the textarea below it up or down depending on the step. Sized to
  // comfortably fit the tallest measured card across Steps 2-9 (~376px on a
  // 390px-wide viewport) with a small buffer. md:min-h-0 resets this on
  // desktop-width layouts, where the wider 672px-max container already
  // wraps prompts to fewer lines. Applies uniformly to every step here
  // (including Step 9/isSummary) — deliberately no special-casing.
